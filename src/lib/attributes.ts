...
export const ATTRIBUTE_LABELS: Record<AttributeCategory, string> = {
  make: "الشركات المصنعة / الماركات",
  category: "أنواع الهيكل / الفئات",
  color: "الألوان الخارجية",
  engine_type: "أنواع الوقود والمحركات",
...
export const FALLBACK_ATTRIBUTES: Record<AttributeCategory, string[]> = {
  ...
  category: ["Sedan", "SUV", "Coupe", "Sports", "Electric", "Truck", "Convertible", "Hatchback"],
  color: [
    "Pearl White",
    "Alpine White",
    "Guards Red",
...
/**
 * Loads dynamic dropdown values (makes, body types, colors, engine types…)
 * from Supabase, merged with fallback defaults. New categories a dealer adds
 * from the Attributes admin page show up in the public filters immediately.
 */
export function useAttributes(category: AttributeCategory) {
  ...
  const add = useCallback(
    async (value: string) => {
      ...
      setValues((prev) => (prev.includes(trimmed) ? prev : [...prev, trimmed].sort()));
    },
    [category],
  );
