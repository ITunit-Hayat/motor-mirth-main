...
import { Heart, SlidersHorizontal, X, Sparkles, ArrowRight, Zap } from "lucide-react";
...
export const Route = createFileRoute("/cars/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Search and filter our premium luxury and certified pre-owned inventory." },
    ],
  }),
  component: CarsPage,
});
...
function CarsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"All" | "Available" | "Featured">("All");
  ...
  }, [cars, statusFilter, make, category, transmission, fuel, year, maxPrice, maxMileage, query, sort, favsOnly, favs]);
    ...
    <PublicLayout>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10">
        ...
        <div className="mb-8 p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-accent/15 via-card to-card border border-accent/30 flex items-center justify-between gap-4 flex-wrap shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-sm sm:text-base">
                {locale === "ar" ? "تريد بيع أو استبدال سيارتك الحالية؟" : "Looking to sell or trade in your current vehicle?"}
              </div>
              <div className="text-xs text-muted-foreground">
                {locale === "ar" ? "احصل على تقييم فوري وعرض نقدي خلال دقائق" : "Get an instant valuation and guaranteed cash offer today."}
              </div>
            </div>
          </div>
          <Link
          ...
          >
            {t("navSellCar")} <ArrowRight className="h-3.5 w-3.5 rtl:rotate-180" />
          </Link>
        </div>

        <div className="flex items-end justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-3xl sm:text-4xl font-display font-bold">{t("allVehicles")}</h1>
