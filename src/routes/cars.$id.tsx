...
function CarDetails() {
  ...
  const isHidden =
    car && (car.status === "Draft" || car.status === "PendingReview");
  const canPreviewHidden =
    isHidden && (isSuperAdmin || (car?.sellerId && car.sellerId === userId));

  if (!car || (isHidden && !canPreviewHidden)) {
    return (
      <PublicLayout>
  ...
  const isMemberListing = !!car.sellerId;
  const contactWhatsapp = isMemberListing
    ? car.sellerPhone || ""
    : site.whatsapp;
  const waText = encodeURIComponent(
    `Hi, I'm interested in ${car.title} (ID: ${car.id})`,
  );
  const waHref = `https://wa.me/${contactWhatsapp.replace(/\D/g, "")}?text=${waText}`;
    ...
    <PublicLayout>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 pb-32 lg:pb-12">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1.4fr_1fr]">
          ...
          <div>
            ...
            <div className="mt-8 flex items-center gap-2 flex-wrap">
              <button
                ...
                className={cn(
                  "inline-flex items-center gap-2 h-10 px-4 rounded-full border text-sm font-semibold",
                  isFav
              ...
              <button
                onClick={() => {
                  if (!isCmp && cmp.length >= COMPARE_LIMIT) {
                  ...
                  <div className="bg-card border border-border rounded-xl p-4">
                    ...
                    <div className="mt-1 font-semibold flex items-center gap-2">
                      <span
                        ...
                        style={{
                          background: car.color.toLowerCase().includes("black")
                            ? "#111"
                            : car.color.toLowerCase().includes("white")
                              ? "#eee"
                              : car.color.toLowerCase().includes("red")
                                ...
                                : car.color.toLowerCase().includes("blue")
                                  ...
                                  : car.color.toLowerCase().includes("silver")
                                    ? "#a3a3a3"
                                    : car.color.toLowerCase().includes("green")
                                      ? "#16a34a"
                                      : "#6b7280",
                        }}
                      />
          ...
          >
            {isMemberListing ? (
              <div className="bg-card border-2 border-accent/40 rounded-2xl p-5 sm:p-6 shadow-elegant space-y-4">
                <span className="inline-flex items-center gap-1.5 text-xs font-bold text-accent uppercase tracking-wider">
                  <User className="h-3.5 w-3.5" /> إعلان من بائع فردي
                </span>
                ...
                <div className="pt-3 border-t border-border/80 space-y-1.5">
                  <div className="text-sm font-semibold">
                    {car.sellerName || "البائع"}
                  </div>
                  {(car.wilaya || car.commune) && (
                    <div className="text-xs text-muted-foreground flex items-center gap-1">
                      <MapPin className="h-3 w-3" /> {car.wilaya}
                      {car.commune ? ` — ${car.commune}` : ""}
                    </div>
                  )}
                </div>
                <div className="grid gap-2.5">
                  {car.sellerPhone && (
                    <a
                      ...
                      className="w-full h-12 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm flex items-center justify-center gap-2"
                    >
                  ...
                  {car.sellerPhone && (
                    <a
                      ...
                      className="w-full h-11 rounded-xl border border-input font-semibold text-sm flex items-center justify-center gap-2"
                    >
                      اتصال مباشر: {car.sellerPhone}
                    </a>
                  )}
                </div>
              ...
              <>
                <div className="bg-gradient-to-br from-card via-card to-accent/10 border-2 border-accent/40 rounded-2xl p-5 sm:p-6 shadow-elegant space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2.5">
                    <button
                      type="button"
                      onClick={() => setPurchaseModal("reserve")}
                ...
                <div className="bg-card border border-border rounded-2xl p-6 shadow-card">
                  ...
                  {!submitted && (
                    <div className="mt-4 grid grid-cols-3 gap-1.5">
                      {(
                        [
                          { value: "Purchase", label: t("reqTypePurchase") },
                          { value: "TestDrive", label: t("reqTypeTestDrive") },
                          { value: "Financing", label: t("reqTypeFinancing") },
                        ] as const
                      ).map((opt) => (
                        <button
                          key={opt.value}
        ...
        <div className="fixed lg:hidden bottom-0 inset-x-0 z-40 bg-card/95 backdrop-blur border-t border-border p-3 flex gap-2 shadow-elegant">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="h-12 w-12 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white grid place-items-center shrink-0"
      ...
      {/* CAR PURCHASE & RESERVATION MODAL */}
      {car && (
        <CarPurchaseModal
          car={car}
          isOpen={!!purchaseModal}
          initialMode={purchaseModal || "reserve"}
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-6 shadow-card">
      ...
      <button
        type="button"
        onClick={onApply}
        className="mt-3 w-full h-10 rounded-lg border border-accent text-accent font-bold text-sm hover:bg-accent/10"
      >
        {t("applyFinance")}
      </button>
    </div>
