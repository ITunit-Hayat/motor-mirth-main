...
function AdminHome() {
  ...
  const soldThisMonth = cars.filter((c) => {
    ...
    return true; // status history isn't timestamped in this schema — shown as a running total
  }).length;

  const stats = [
    { label: "Total Cars Listed", value: cars.length, Icon: Car },
    { label: "Sold", value: soldThisMonth, Icon: TrendingUp },
    { label: "Unread Requests", value: newOrders, Icon: ClipboardList },
    ...
    <AdminLayout>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2">
            Dashboard <LiveBadge count={newOrders} />
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Real stats from this site's visitors (tracked live in-browser).
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {statusCounts.map((sc) => (
          <span
            key={sc.status}
            className="text-xs font-semibold px-3 py-1.5 rounded-full bg-secondary border border-border"
          >
            {sc.status}: <span className="text-accent">{sc.count}</span>
          </span>
        ))}
      </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5 shadow-card">
          ...
          {a.topPaths.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">
              No page views tracked yet — browse the public site to generate data.
            </p>
          ) : (
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            {topCars.map(({ car, count }) => (
              <Link
                ...
                to="/cars/$id"
                params={{ id: car!.id }}
                className="p-3 rounded-xl bg-secondary hover:shadow-card transition"
              >
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 max-w-4xl">
        <Link
          to="/admin/cars"
          className="bg-card border border-border rounded-xl p-5 hover:shadow-card transition"
        >
        ...
        <Link
          ...
          className="bg-card border border-border rounded-xl p-5 hover:shadow-card transition"
        >
          <div className="mt-2 font-semibold">Makes, Colors & More →</div>
        </Link>
        <Link
          to="/admin/settings"
          className="bg-card border border-border rounded-xl p-5 hover:shadow-card transition"
