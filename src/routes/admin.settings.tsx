...
function SettingsPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    setSaved(f);
    toast.success("تم حفظ الإعدادات وتطبيقها على كامل الموقع فورياً!");
  };
  ...
  const input = "mt-1 w-full h-11 px-3.5 rounded-xl bg-background border border-input text-sm focus:ring-2 focus:ring-accent outline-none text-foreground";
  ...
  return (
    <AdminLayout>
      ...
      <p className="text-muted-foreground text-sm mt-1">
        التحكم في هوية المعرض ومعلومات التواصل وكلمة مرور لوحة الإدارة. يتم تطبيق أي تعديل فورياً.
      </p>

      <form onSubmit={submit} className="mt-8 grid gap-6 lg:grid-cols-2 max-w-4xl">
        <div className="bg-card border border-border rounded-2xl p-6 shadow-card space-y-4">
          <h2 className="font-bold text-foreground text-base">هوية الموقع والمعرض</h2>
          <div>
          ...
          <label className="flex items-center gap-2.5 text-sm cursor-pointer pt-1">
            ...
            />
            <span className="font-medium">تفعيل شريط العروض والخصومات وقسم الصفقات الساخنة</span>
          </label>
        </div>

        <div className="bg-card border border-border rounded-2xl p-6 shadow-card space-y-4">
          <h2 className="font-bold text-foreground text-base">معلومات التواصل (المعروضة للعملاء)</h2>
          <div>
            <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
          ...
          <div>
            <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
              <MessageCircle className="h-3.5 w-3.5" /> رقم الواتساب الرسمي (مع رمز الدولة الدولي)
            </label>
            <input
              value={f.whatsapp}
        ...
        <button
          type="submit"
          className="lg:col-span-2 h-12 rounded-xl bg-accent text-accent-foreground font-bold inline-flex items-center justify-center gap-2 max-w-xs shadow-md hover:opacity-90 transition-opacity cursor-pointer text-sm"
        >
          <Save className="h-4 w-4" /> حفظ بيانات الموقع
        </button>
      </form>
      ...
      <div className="mt-8 max-w-4xl bg-card border border-border rounded-2xl p-6 shadow-card">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          يتم الدخول للوحة التحكم مباشرة بكلمة المرور دون الحاجة لحساب أو بريد إلكتروني.
        </p>

        <form onSubmit={handlePasswordChange} className="mt-5 max-w-md space-y-3">
          <div>
          ...
          {passSaved && (
            <div className="flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 font-semibold pt-1">
              <CheckCircle2 className="h-4 w-4" /> تم حفظ وتطبيق كلمة المرور الجديدة بنجاح!
            </div>
          )}

          <div className="text-xs text-muted-foreground pt-1">
            كلمة المرور الفعالة حالياً: <span className="font-mono font-bold text-foreground" dir="ltr">{currentPasswordHint}</span>
          </div>
        </form>
      </div>
    </AdminLayout>
