 ...
 */
async function compressAndWatermark(
  file: File,
...
async function uploadOne(file: File, watermarkText: string): Promise<string> {
  ...
  // Bucket missing or policy blocked it — keep the compressed photo inline
  // (base64) so the admin NEVER loses their work, and warn once.
  console.warn("car-images upload failed:", error.message);
  toast.warning(
    "تعذر الرفع إلى التخزين السحابي — تم حفظ الصورة محلياً. شغّل supabase/004_fix_stats_and_storage.sql في Supabase لتفعيل الرفع الدائم.",
  );
      ...
      >
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-2 text-sm text-muted-foreground">
            <Loader2 className="h-6 w-6 animate-spin text-accent" /> Compressing
            & uploading…
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2 text-sm text-muted-foreground">
            ...
            <span className="text-xs">
              Auto-compressed and watermarked · drag thumbnails to reorder ·
