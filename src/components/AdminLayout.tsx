...
function AdminGate({ children }: { children: ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen grid place-items-center bg-background text-sm text-muted-foreground" dir="rtl">
  ...
  if (needsLogin) {
    return (
      <div className="min-h-screen grid place-items-center bg-gradient-hero p-6" dir="rtl">
        <div className="w-full max-w-md bg-card border border-border rounded-3xl shadow-elegant p-8 text-center relative overflow-hidden">
          ...
          <h1 className="mt-5 font-display text-2xl font-bold text-foreground">لوحة تحكم الإدارة</h1>
          ...
          >
            ...
            >
              ...
              <span>{submitting ? "جاري التحقق..." : "دخول إلى لوحة التحكم"}</span>
            </button>
          </form>

          <div className="mt-5 p-3 rounded-xl bg-muted/60 border border-border/80 text-xs text-muted-foreground">
            💡 كلمة المرور الافتراضية: <span className="font-mono font-bold text-foreground" dir="ltr">admin123</span>
          </div>
...
function AdminShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "orders" }, (payload) => {
        const name = (payload.new as { customer_name?: string })?.customer_name ?? "عميل";
        toast.message("طلب جديد وارد", { description: `قام ${name} بتقديم طلب جديد للتو.` });
        setNewCount((n) => n + 1);
      })
      .subscribe();
    return () => {
  ...
  const nav = [
    ...
    { to: "/admin/orders", label: "الطلبات والحجوزات", Icon: ClipboardList, badge: newCount },
      ...
      <header className="bg-card border-b border-border sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link to="/admin" className="font-display font-bold text-lg shrink-0 flex items-center gap-2">
          ...
          <nav className="flex items-center gap-1 overflow-x-auto no-scrollbar">
            {nav.map((n) => (
              <Link
                ...
                activeProps={{ className: "text-accent bg-accent/10 font-bold" }}
          ...
          <div className="flex items-center gap-2 border-r border-border pr-3">
            ...
            >
              ...
              {role === "SuperAdmin" ? "مسؤول عام (Super Admin)" : "وكيل مبيعات"}
            </span>
            ...
            >
              <LogOut className="h-3.5 w-3.5" /> <span className="hidden sm:inline">خروج</span>
            </button>
          </div>
        </div>
      </header>
